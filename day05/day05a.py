...
with each kind of fertilizer, and so on. Every type of seed, soil, fertilizer and so on is identified with a 
number, but numbers are reused by each category - that is, soil 123 and fertilizer 123 aren't necessarily 
...
entire ranges of numbers that can be converted. Each line within a map contains three numbers: the destination 
...
line means that the source range starts at 98 and contains two values: 98 and 99. The destination range is the 
same length, but it starts at 50, so its two values are 50 and 51. With this information, you know that seed 
...
So, the entire list of seed numbers and their corresponding soil numbers looks like this:
    seed  0 1 ... 48 49 50 51 ... 96 97 98 99
    soil  0 1 ... 48 49 52 53 ... 98 99 50 51
...
with open("input.txt") as f:
    data = f.read().replace("seeds: ", "seeds:\n").strip()

categories = map(lambda x: x.split(":\n")[1], data.split("\n\n"))
seeds = [*map(int, next(categories).split())]
sets = map(lambda x: [*map(str.split, x.split("\n"))], categories)

for comparison in sets:
    for xi, seed in enumerate(seeds[::]):
        for item in comparison:
            dest, source, span = map(int, item)
            if seed in range(source, source + span):
                seeds[xi] = dest + seed - source
                break

print(min(seeds))
    
#lowest location is 650599855
