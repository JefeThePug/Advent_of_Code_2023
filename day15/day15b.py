...
through the box. The side of each box has a panel that opens to let you to insert or remove lenses as necessary.
...
is present in the box. Then, move any remaining lenses as far forward in the box as they can go without changing 
...
Here is the contents of every box after each step in the example initialization sequence above:
    After "rn=1":             After "cm-":              After "qp=3":             After "cm=2":
    Box 0: [rn 1]             Box 0: [rn 1]             Box 0: [rn 1]             Box 0: [rn 1] [cm 2]
                                                        Box 1: [qp 3]             Box 1: [qp 3]
      
    After "qp-":              After "pc=4":             After "ot=9":             After "ab=5":
    Box 0: [rn 1] [cm 2]      Box 0: [rn 1] [cm 2]      Box 0: [rn 1] [cm 2]      Box 0: [rn 1] [cm 2]
                              Box 3: [pc 4]             Box 3: [pc 4] [ot 9]      Box 3: [pc 4] [ot 9] [ab 5]
    
    After "pc-":              After "pc=6":                    After "ot=7":
    Box 0: [rn 1] [cm 2]      Box 0: [rn 1] [cm 2]             Box 0: [rn 1] [cm 2]
    Box 3: [ot 9] [ab 5]      Box 3: [ot 9] [ab 5] [pc 6]      Box 3: [ot 7] [ab 5] [pc 6]
...
for step in steps:
    label, focal = re.match(r"(\w+)(?:=|-)(\d?)", step).groups()
    box = reduce(lambda x, y: (x + ord(y))*17%256, label, 0)
    if focal:
...
total = 0
for b, lens in enumerate(boxes, 1):
    total += sum(b*s*f for s, f in enumerate(lens.values(), 1))

print(total)

# focusing power is 284132
