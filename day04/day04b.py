...
class Card:
    def __init__(self, s: str):
        cardno, data = s.split(":")
        self.num = int(cardno.split()[1])
        winning, hand = map(str.split, data.split("|"))
        self.num_won = len(set(winning) & set(hand))
        
        
with open("input.txt") as f:
    s = f.read()

card_pile = [Card(card) for card in s.split("\n")[:-1]]

for card in card_pile:
    for i in range(card.num_won):
        card_pile.append(card_pile[card.num + i])
