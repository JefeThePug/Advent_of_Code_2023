...
have a delicate ecosystem; you might as well take some ecological readings while you wait. Maybe you can report 
any environmental instabilities you find to someone so the oasis can be around for the next sandstorm-worn 
traveler.
...
Then, following the same process as before, work out the next value in each sequence from the bottom up:
    1   3   6  10  15  21        1   3   6  10  15  21  28
...
def get_next(line: str) -> int:
    line = [*map(int, line.split())]
    triangle = [line]
    while set(line) != {0}:
        line = [b - a for a, b in zip(line, line[1:])]
        triangle.append(line)
    right_edge = [*accumulate(t[-1] for t in triangle[::-1])]
    return right_edge[-1]
