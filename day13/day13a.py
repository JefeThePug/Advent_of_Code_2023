...
the ash at strange angles. Because the terrain is all one color, it's hard to tell where it's safe to walk or 
...
The second pattern reflects across a horizontal line instead:
    ...
    6 ..##..### 6
    7 #....#..# 7
This pattern reflects across the horizontal line between rows 4 and 5. Row 1 would reflect with a hypothetical 
row 8, but since that's not in the pattern, row 1 doesn't need to match anything. The remaining rows match: row 2 
...
def get_middle(grid: List[str]) -> int:
    for i in range(len(grid) - 1):
        if grid[i] == grid[i + 1]:
            if all(x == y for x, y in zip(grid[:i + 1][::-1], grid[i + 1:])):
                return i + 1    
    return 0
...
print(sum(get_middle([*zip(*g.split("\n"))]) + 100*get_middle([*map(list, g.split("\n"))]) for g in grids))
    
    
# summarization is 33195
