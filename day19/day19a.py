...
someone stole from Desert Island; since you're already going that direction, it would help them a lot if you 
...
a formidable heap. Don't worry, though - a group of Elves is already here organizing the parts, and they have a 
system.
...
workflow has a name and contains a list of rules; each rule specifies a condition and where to send the part if 
the condition is true. The first rule that matches the part being considered is applied immediately, and the 
part moves on to the destination described by the rule. (The last rule in each workflow has no condition and 
...
The workflows are listed first, followed by a blank line, then the ratings of the parts the Elves would like 
you to sort. All parts begin in the workflow named in. In this example, the five listed parts go through the 
following workflows:
...
7540 for the part with x=787, 4623 for the part with x=2036, and 6951 for the part with x=2127. Adding all of 
...
class Path:
    def __init__(self, name: str, rules: str):
        self.name = name
        self.conditions = rules.split(",")
        
    def get(self, item: Item) -> str:
        for c in self.conditions:
            if ":" in c:
                condition, output = c.split(":")
                if eval(f"item.{condition}"):
                    return output
            else:
                return c
...
with open("input.txt") as f:
    paths, items = map(str.split, f.read().strip().split("\n\n"))

items = [Item(*re.findall(r"\d+", i)) for i in items]
paths = {(x:= Path(*p[:-1].split("{"))).name: x for p in paths}
xmas = 0
