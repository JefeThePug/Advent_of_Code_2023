...
the sand, but the machines have broken down because Desert Island recently stopped receiving the parts they need 
to fix the machines.
...
hand consists of five cards labeled one of A, K, Q, J, T, 9, 8, 7, 6, 5, 4, 3, or 2. The relative strength of 
...
iffer, the hand with the higher second card wins; otherwise, continue with the third card in each hand, then the 
...
class Hand:
    def __init__(self, hand: str, bid: str):
        ...
        self.bid = int(bid)
        self.handvisual = hand
        self.hand = [card_vals[c] for c in hand]
        self.type = 1
        n = len(set(self.hand))
        self.type += sum(n <= x for x in range(4, 0, -1))
        self.type += sum(any(self.hand.count(x) in (5, 4, 3)[:i] for x in self.hand) for i in range(1, 4))
        self.type -= (n == 1)
