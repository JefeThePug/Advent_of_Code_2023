...
this process until all of the nodes you're currently on end with Z. (If only some of the nodes you're on end with
...
with open("input2.txt") as f:
    ...
    node_map = {node: dirs.split(", ") for node, dirs in nodes}

pos = [k for k in node_map if k.endswith("A")]
lcms = []

for c, d in enumerate(directions, 1):
    pos = [node_map[p][d] for p in pos]
    for p in pos[:]:
        if p.endswith("Z"):
            lcms.append(c)
            pos.remove(p)
